...
I/O burst가 많음: I/O bound (대체로 이쪽 빈도가 높음)
...
프로세스가 종료 상태나 대기 상태가 될 때까지 CPU를 계속 사용하고 있음

### 선점형 스케줄링

스케줄러에 의해 프로세스가 선점할 수 있음
...
dispatcher는 context switching마다 dispatch가 발생하므로, 가능한 한 매우 빨라야 한다.
...
- 대기 시간(waiting time)
    ...
    - ready queue에서 대기하는 데 걸리는 주기의 합
- 응답 시간(response time)
    - 응답하는 데 걸리는 시간
...
평균 waiting time이 대체로 크며, CPU-burst time이 다양할 경우 편차가 매우 큼

비선점형 스케줄링

### Convoy Effect(호위 효과)

작업 시간이 긴 프로세스가 CPU를 사용하고 있을 경우, 상대적으로 짧은 프로세스는 끝날 때까지 대기하고 있는 현상

낮은 CPU 및 device 활용을 가져옴
...
다음 CPU burst 길이를 알 수 있는 방법이 없음 → 구현 난이도 어려움
...
ready queue는 circular queue로 취급

스케줄러는 ready queue를 순회하는 중, 각 프로세스에 1 time quantum까지 시간 간격으로 CPU 할당
